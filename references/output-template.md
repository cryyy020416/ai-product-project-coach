...
如果任一步骤不通过，停止生成后续方案，改用 `stage-gate-prompts.md` 的不通过输出格式。

| 步骤 | 结论 | 依据 | 风险/下一步 |
| --- | --- | --- | --- |
| 1. 场景与真实需求 | 通过 / 有条件通过 / 不通过 |  |  |
| 2. AI 介入必要性 | 通过 / 有条件通过 / 不通过 |  |  |
| 3. AI 产品链路可落地性 | 通过 / 有条件通过 / 不通过 |  |  |
| 4. 评测集与 MECE 标准 | 通过 / 有条件通过 / 不通过 |  |  |
| 5. Badcase 迭代闭环 | 通过 / 有条件通过 / 不通过 |  |  |
| 6. 证据与简历边界 | 通过 / 有条件通过 / 不通过 |  |  |
